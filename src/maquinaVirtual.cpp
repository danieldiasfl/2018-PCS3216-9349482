...
void maquinaVirtual::interpreta(string instruct)
{
    string instrucao = instruct;
    ...
    acumulador2 = complemento2INT(acumulador, sinalAc);
    ...
    else if(operador == "4"){ //soma no acumulador
        if(indireto == 1){
            string opIndireto;
            opIndireto = memoria[operando];
            operando = strtol(opIndireto.c_str(), NULL, 16);
